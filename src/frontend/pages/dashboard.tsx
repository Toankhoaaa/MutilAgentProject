      ...
      <Layout user={user ?? null}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-7">
          <div>
            ...
            <p className="text-sm text-zinc-400 mt-0.5">AI-powered email pipeline</p>
          </div>
        ...
        {/* Content grid: analytics chart + agent/scheduler sidebar */}
        <div className="content-grid">
          {/* Category distribution */}
          <div className="card col-span-1 lg:col-span-2">
            <h2 className="text-base font-semibold tracking-tight text-zinc-800 mb-5">
              Category Distribution
            </h2>
            <CategoryChart
          ...
          {/* Sidebar: scheduler + last agent run */}
          <div className="card">
            {/* Scheduler */}
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-semibold tracking-tight text-zinc-800">Auto Scheduler</h2>
              {schedulerStatus && (
                <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full
                  ${schedulerStatus.is_running
                    ? "bg-emerald-50 text-emerald-700"
                    : "bg-zinc-100 text-zinc-400"}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${schedulerStatus.is_running ? "bg-emerald-500 animate-pulse" : "bg-zinc-400"}`} />
            ...
            {schedulerStatus ? (
              <div className="space-y-2 mb-4">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="text-zinc-700 font-medium">{schedulerStatus.interval_minutes} min</span>
                </div>
              ...
              <div className="space-y-2 mb-4">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="h-3.5 bg-zinc-100 rounded animate-pulse" />
                ))}
              </div>
            )}
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`btn-secondary text-xs py-1 px-2 flex-1 disabled:opacity-50 ${
                  schedulerStatus?.is_running
